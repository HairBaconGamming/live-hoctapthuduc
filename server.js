...
const peerServer = ExpressPeerServer(server, {
  ...
  generateClientId: () => { // More robust client ID generation if needed
    return uuidv4();
  }
});
...
if (process.env.NODE_ENV !== 'production' && JWT_SECRET === "fallback_super_secret_key_for_dev_only") {
    console.warn("\n********************************************************************");
    console.warn("WARNING: Using fallback JWT_SECRET. This is INSECURE for production!");
...
function simulateLogin(req, res, next) {
    ...
    if (req.query.simulatedUser) {
        req.user = { // This structure should match what your actual login provides
    ...
    } else if (!req.path.startsWith('/api/') && !req.path.startsWith('/peerjs') && !req.path.includes('.') && req.path !== '/live' && req.path !== '/') {
...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            ...
            "connect-src": [ // IMPORTANT for Socket.IO and PeerJS
                "'self'",
...
app.post("/api/generateRoomToken", (req, res) => {
    ...
    if (!userId || !username || !roomIdToAccess) {
        return res.status(400).json({ error: "User ID, username, and roomIdToAccess are required." });
    }
...
app.post("/api/createStream", (req, res) => {
  ...
  const roomOwnerId = req.body.roomOwnerId || req.user?._id || `defaultOwner_${Date.now()}`;
  const roomOwnerName = req.body.roomOwnerName || req.user?.username || "Default Streamer";
  ...
  const newRoom = {
    ...
    peerConfig: {
        ...
        // key: 'peerjs', // Optional: if you configured a key on your PeerServer
        // If not on Glitch, adjust host/port/secure accordingly
        // For local dev: host: 'localhost', port: PORT (e.g. 3001), path: '/peerjs/myapp', secure: false
    },
    glitchProjectUrl: `https://${process.env.PROJECT_DOMAIN}.glitch.me` // For redirects
  };
  liveRooms.push(newRoom);
  console.log("✅ Room created:", { id: newRoom.id, owner: newRoom.owner, title: newRoom.title });
  res.json({ success: true, liveStreamUrl: newRoom.liveStreamUrl, roomId: newRoom.id });
});
...
app.get("/api/rooms", (req, res) => {
  const roomsToDisplay = liveRooms
    ...
    .map(room => {
      ...
      const onlineTime = `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
      return {
        id: room.id,
        owner: room.owner,
        title: room.title,
...
app.get("/room/:id", checkHoctapAuth, (req, res) => {
  ...
  if (!room) {
    return res.status(404).render("errorPage", { message: "Phòng live không tồn tại hoặc đã kết thúc.", projectUrl: `https://${process.env.PROJECT_DOMAIN}.glitch.me` });
  }

  // Check if user is banned BEFORE rendering
  if (room.bannedViewers.includes(req.user.username)) {
    return res.status(403).render("errorPage", { message: "Bạn đã bị chặn khỏi phòng live này.", projectUrl: room.glitchProjectUrl });
  }
  ...
  if (room.ownerid.toString() === req.user.userId.toString()) {
    ...
    if (room.hostSocketId && room.hostSocketId !== req.user.socketId /* will be set by socket handler */) {
        ...
        if (oldHostSocket) {
            ...
            console.log(`Disconnected old host socket ${room.hostSocketId} for room ${room.id}`);
        }
    }
    // room.hostSocketId will be updated in the 'joinRoom' socket event
    console.log(`Host ${req.user.username} is accessing their room ${room.id}.`);
    res.render("streamer", {
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", ({ roomId, username, peerId }) => {
    ...
    console.log(`${username} (Socket: ${socket.id}, Peer: ${peerId || 'N/A'}) joined room ${roomId}`);

    if (username === room.owner) { // Host connected
      if (room.hostSocketId && room.hostSocketId !== socket.id) {
        const oldHostSocket = io.sockets.sockets.get(room.hostSocketId);
        if (oldHostSocket) {
          oldHostSocket.emit("forceEndStream", "Phiên live của bạn đã được tiếp tục từ một thiết bị/tab khác.");
          oldHostSocket.disconnect(true);
          console.log(`Disconnected old host socket ${room.hostSocketId} for room ${room.id} as new host ${socket.id} joined.`);
        }
      }
      room.hostSocketId = socket.id;
      room.isLive = true;
      io.to(roomId).emit("hostJoined"); // Notify viewers host is present
      console.log(`Host ${username} (Socket: ${socket.id}) confirmed for room ${roomId}`);

      // Send current whiteboard visibility state to rejoining host
      socket.emit('wb:toggleVisibility', { isVisible: room.isWhiteboardActive });
    ...
    } else { // Viewer connected
      const existingViewer = room.viewersList.find(v => v.username === username);
      if (!existingViewer) {
        room.viewersList.push({ username: username, peerId: peerId, canDraw: false });
        room.viewers++;
      } else {
      ...
      if (!room.isLive) {
        ...
        io.to(room.hostSocketId).emit("newViewer", { viewerId: peerId, username: username });
      }
      ...
      if (viewerData) {
          socket.emit('wb:permissionUpdate', { viewerUsername: username, canDraw: viewerData.canDraw });
      }
  ...
  socket.on("newViewer", ({ viewerId, roomId, username }) => {
    const room = findRoom(roomId);
    if (room && username === socket.username) { // Ensure it's the correct viewer updating their peerId
        socket.peerId = viewerId; // Update peerId on their socket session
        const viewerInList = room.viewersList.find(v => v.username === username);
        if (viewerInList) {
            viewerInList.peerId = viewerId;
        }
        if (room.hostSocketId && room.isLive) {
            console.log(`Viewer ${username} (Peer: ${viewerId}) is ready, notifying host ${room.hostSocketId}`);
            io.to(room.hostSocketId).emit("newViewer", { viewerId: viewerId, username: username });
        }
    }
  });


  socket.on("chatMessage", ({ roomId, message }) => { // message contains {username, content, messageType, timestamp}
    const room = findRoom(roomId);
    if (room) {
  ...
  socket.on("banViewer", ({ roomId, viewerUsername }) => {
    const room = findRoom(roomId);
    if (room && socket.username === room.owner) {
      if (!room.bannedViewers.includes(viewerUsername) && viewerUsername !== room.owner) {
        ...
        io.in(roomId).allSockets().then(socketsInRoom => {
            socketsInRoom.forEach(socketIdInRoom => {
                ...
                if (targetSocket && targetSocket.username === viewerUsername) {
                    targetSocket.emit("banned", "Bạn đã bị ban khỏi phòng live này.");
                    targetSocket.leave(roomId); // Remove from room's broadcast
                    targetSocket.disconnect(true); // Force disconnect
                    console.log(`Force disconnected banned viewer ${viewerUsername} (Socket: ${socketIdInRoom}) from room ${roomId}`);
                }
            });
        });
        // Update lists
        room.viewersList = room.viewersList.filter(v => v.username !== viewerUsername);
        ...
        if (room.hostSocketId) {
            ...
            io.to(room.hostSocketId).emit("viewerBanned", `${viewerUsername} đã bị ban.`);
        }
        console.log(`Viewer ${viewerUsername} banned from room ${roomId} by ${socket.username}`);
      }
    }
  });
  ...
  socket.on("unbanViewer", ({ roomId, viewerUsername }) => {
    ...
    if (room && socket.username === room.owner) {
      ...
      console.log(`Viewer ${viewerUsername} unbanned from room ${roomId} by ${socket.username}`);
    }
  });

  socket.on("getBannedList", ({ roomId }) => {
    const room = findRoom(roomId);
    if (room && socket.id === room.hostSocketId) { // Only host should request/receive this directly
  ...
  socket.on("getViewersList", ({ roomId }) => {
    const room = findRoom(roomId);
    if (room && socket.id === room.hostSocketId) { // Only host
      socket.emit("updateViewersList", { viewers: room.viewersList });
    }
  });

  socket.on("streamEnded", ({ roomId }) => { // From streamer when they stop stream (not full room end)
    const room = findRoom(roomId);
  ...
  socket.on("wb:draw", ({ roomId, drawData }) => {
    const room = findRoom(roomId);
    if (room) {
        const userCanDraw = (socket.username === room.owner) || (room.viewersList.find(v => v.username === socket.username)?.canDraw);
        if (userCanDraw) {
            // Add to server's history for this room
            room.whiteboardHistory.push(drawData);
            if (room.whiteboardHistory.length > 1000) { // Limit history size
                room.whiteboardHistory.splice(0, room.whiteboardHistory.length - 1000);
            }
            // Broadcast to other clients
            socket.to(roomId).emit("wb:draw", { username: socket.username, drawData });
        } else {
            console.warn(`User ${socket.username} tried to draw without permission in room ${roomId}.`);
        }
    }
  });
  socket.on("wb:drawShape", ({ roomId, shapeData }) => {
    ...
    if (room) {
        ...
        if (userCanDraw) {
            ...
            if (room.whiteboardHistory.length > 1000) room.whiteboardHistory.splice(0, room.whiteboardHistory.length - 1000);
            socket.to(roomId).emit("wb:drawShape", { username: socket.username, shapeData });
        }
    }
  });
  ...
  socket.on("wb:clear", ({ roomId }) => {
    const room = findRoom(roomId);
    if (room && socket.username === room.owner) { // Only host can clear for everyone
      room.whiteboardHistory = [{type: 'clear', timestamp: Date.now(), drawnBy: socket.username}]; // Store clear action
      io.to(roomId).emit("wb:clear"); // Use io.to to include sender (host) for UI update
      console.log(`Whiteboard cleared in room ${roomId} by host ${socket.username}`);
    }
  });
  ...
  socket.on("wb:requestInitialState", ({ roomId }) => {
    ...
    if (room) {
      ...
      if (room.hostSocketId && io.sockets.sockets.get(room.hostSocketId)) {
        console.log(`Relaying wb:requestInitialState from ${socket.id} to host ${room.hostSocketId} for room ${roomId}`);
        ...
        console.log(`Host not present or unresponsive, sending stored WB history to ${socket.id} for room ${roomId}`);
        socket.emit("wb:initState", { history: room.whiteboardHistory });
      } else {
  ...
  socket.on("wb:syncStateToViewer", ({ targetViewerId, history }) => {
    ...
    if (room && socket.id === room.hostSocketId) { // Only host can sync
      if (targetViewerId) {
        console.log(`Server forwarding whiteboard history from host ${socket.username} to viewer ${targetViewerId}`);
  ...
  socket.on("wb:toggleGlobalVisibility", ({ roomId, isVisible }) => {
    ...
    if (room && socket.username === room.owner) {
      ...
      console.log(`Whiteboard global visibility for room ${roomId} set to ${isVisible} by host ${socket.username}`);
      if(isVisible && (!room.whiteboardHistory || room.whiteboardHistory.length === 0)){
        // If turning on and history is empty, might send a "fresh start" or rely on host drawing.
      }
    }
  });

  socket.on("wb:toggleViewerDrawPermission", ({ roomId, viewerUsername, canDraw }) => {
    const room = findRoom(roomId);
    if (room && socket.username === room.owner) {
      const viewer = room.viewersList.find(v => v.username === viewerUsername);
      if (viewer) {
        viewer.canDraw = canDraw;
        io.to(roomId).emit("wb:permissionUpdate", { viewerUsername, canDraw }); // Notify all, including the specific viewer
        // Host's own list will be updated via its client-side `updateViewersList` handler after this.
        io.to(room.hostSocketId).emit("updateViewersList", { viewers: room.viewersList });
        console.log(`Draw permission for ${viewerUsername} in room ${roomId} set to ${canDraw} by host.`);
      }
    }
  });
        ...
        if (userCanDraw) {
            ...
            movedItemsData.forEach(moved => {
                if (room.whiteboardHistory[moved.index]) {
                    Object.assign(room.whiteboardHistory[moved.index], moved.newItemData);
                }
            });
            // Broadcast to other clients
            socket.to(roomId).emit("wb:moveElements", { username: socket.username, movedItemsData });
        }
    }
});
...
socket.on("wb:deleteElements", ({ roomId, indices }) => {
    ...
    if (room) {
        ...
        if (userCanDraw) {
            ...
            // Broadcast to other clients
            socket.to(roomId).emit("wb:deleteElements", { username: socket.username, indices });
        }
    }
});
  ...
  socket.on("quiz:start", ({ roomId, questionText, options, correctAnswerIndex }) => {
    ...
    if (room && socket.username === room.owner) {
        ...
        room.quiz = { // Reset quiz state for new question
            isActive: true,
            currentQuestionId: questionId,
            currentQuestion: { id: questionId, text: questionText, options, correctAnswerIndex },
            responses: { [questionId]: {} },
            results: { [questionId]: options.reduce((acc, _, i) => { acc[i] = 0; return acc; }, {}) },
            showCorrectAnswer: false,
        };
        io.to(roomId).emit("quiz:newQuestion", { questionId, text: questionText, options });
        console.log(`Quiz started in room ${roomId} by ${socket.username} (QID: ${questionId})`);
    } else if (room) {
  ...
  socket.on("quiz:submitAnswer", ({ roomId, questionId, answerIndex }) => {
    ...
    if (room && room.quiz.isActive && room.quiz.currentQuestionId === questionId && socket.username) {
        ...
        if (userResponses && questionResults) {
            ...
            if (previousAnswer !== undefined && questionResults[previousAnswer] > 0) {
            ...
            if (room.hostSocketId) { // Update host with live results
                io.to(room.hostSocketId).emit("quiz:resultsUpdate", { questionId, results: questionResults });
            }
            socket.emit("quiz:answerSubmitted", { questionId, answerIndex }); // Confirm to submitter
        } else {
            socket.emit("quiz:error", "Câu hỏi không hợp lệ hoặc trắc nghiệm đã kết thúc.");
        }
    } else if (room) {
  ...
  socket.on("quiz:showAnswer", ({ roomId, questionId }) => {
    ...
    if (room && socket.username === room.owner && room.quiz.currentQuestionId === questionId) {
        if (!room.quiz.currentQuestion) {
            socket.emit("quiz:error", "Không có câu hỏi nào đang hoạt động để hiển thị đáp án.");
            return;
        }
  ...
  socket.on("quiz:end", ({ roomId }) => {
    const room = findRoom(roomId);
    if (room && socket.username === room.owner) {
        room.quiz = { isActive: false, currentQuestionId: null, currentQuestion: null, responses: {}, results: {}, showCorrectAnswer: false }; // Full reset
  ...
  socket.on("disconnecting", () => {
    console.log(`Client disconnecting: ${socket.id}, Username: ${socket.username}, Room: ${socket.roomId}`);
    const currentRoomId = socket.roomId; // Get room before leaving
    if (currentRoomId) {
        const room = findRoom(currentRoomId);
        if (room) {
            if (socket.id === room.hostSocketId) { // Host disconnected
                console.log(`Host ${socket.username} disconnected from room ${currentRoomId}. Ending room.`);
                io.to(currentRoomId).emit("roomEnded", "Chủ phòng đã ngắt kết nối.");
                liveRooms = liveRooms.filter(r => r.id !== currentRoomId);
            } else { // Viewer disconnected
                room.viewersList = room.viewersList.filter(v => v.username !== socket.username);
                room.viewers = Math.max(0, room.viewersList.length);
                io.to(currentRoomId).emit("updateViewers", room.viewers);
                io.to(currentRoomId).emit("viewerLeft", `${socket.username} đã thoát phòng.`);
                if (room.hostSocketId) {
                    io.to(room.hostSocketId).emit("updateViewersList", { viewers: room.viewersList });
                    if (socket.peerId) { // If viewer had a peerId
                        io.to(room.hostSocketId).emit("viewerDisconnected", { viewerId: socket.peerId, username: socket.username });
                    }
                }
                console.log(`Viewer ${socket.username} disconnected from room ${currentRoomId}. Remaining viewers: ${room.viewers}`);
            }
        }
    }
  });
...
function cleanupInactiveRooms() {
    ...
    liveRooms = liveRooms.filter(room => {
        ...
        const hostStillConnected = room.hostSocketId && io.sockets.sockets.get(room.hostSocketId);
        if (!hostStillConnected && (now - new Date(room.createdAt).getTime() > twoHours)) {
            console.log(`Cleaning up inactive/orphaned room: ${room.id} (Owner: ${room.owner})`);
            return false;
        }
        return true;
    });
...
server.listen(PORT, () => {
  console.log(`🚀 Live server running on port ${PORT}. Access at http://localhost:${PORT} or your Glitch URL.`);
